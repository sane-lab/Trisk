...
import org.apache.flink.streaming.controlplane.entrypoint.component.StreamManagerDispatcherComponent;
import org.apache.flink.streaming.controlplane.entrypoint.component.StreamManagerDispatcherComponentFactory;
...
public abstract class ClusterControllerEntrypoint implements AutoCloseableAsync, FatalErrorHandler {
	...
	private void runClusterController(Configuration configuration) throws Exception {
		synchronized (lock) {
			...
			smComponent = smDispatcherComponentFactory.create(
				configuration,
				ioExecutor,
				commonRpcService,
				haServices,
			...
			smComponent.getShutDownFuture().whenComplete(
				(ApplicationStatus applicationStatus, Throwable throwable) -> {
					if (throwable != null) {
						shutDownAsync(
							ApplicationStatus.UNKNOWN,
