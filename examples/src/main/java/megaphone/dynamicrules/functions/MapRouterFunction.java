...
@Slf4j
public class MapRouterFunction
//    extends KeyedBroadcastProcessFunction<String, Tuple2<String, String>, String, Keyed<Tuple3<String, String, Long>, String, String>> {
//    extends RichMapFunction<Tuple2<String, String>, Keyed<Tuple3<String, String, Long>, String, String>> {
  extends RichFlatMapFunction<Tuple2<String, String>, Keyed<Tuple3<String, String, Long>, String, String>> {
...
//          "120=A56, 121=A39, 122=A150, 123=A636, 124=A230, 125=A22, 126=A233, 127=A66";
  ...
  String keyGroupToKeyMapStr = "";

  Map<Integer, String> keyGroupToKeyMap = new HashMap<>();
  // key to keygroup map is used for mapping
  private final Map<String, Integer> rulesState = new HashMap<>();
  private final Map<String, Integer> oldBroadcastState = new HashMap<>();
  ...
  private final Map<String, Boolean> affectedKeys = new HashMap<>();
  private final Map<String, Long> frontier = new HashMap<>();
  private final Map<String, List<Tuple2<String, String>>> bufferedTuples = new HashMap<>();
  ...
  @Override
  public void flatMap(Tuple2<String, String> event, Collector<Keyed<Tuple3<String, String, Long>, String, String>> out) throws Exception {
    if (System.currentTimeMillis() - start > 50000 && !reconfig) {
      reconfigStart = System.currentTimeMillis();
      System.out.println("++++++ Reconfig started: " + reconfigStart);
      for (int i = 0; i < keySize; i++) {
        String key = "A" + i;
        rulesState.put(key, i%128);
        if (frontier.get(key) != 0L) {
          affectedKeys.put(key, true);
        }
      }
      reconfig = true;
    }
    if (affectedKeys.containsKey(event.f0) && affectedKeys.get(event.f0) && migratingKey.equals("")) {
      // add a lock to ensure only one migration in progress
      migratingKey = event.f0;
      System.out.println("++++++ migrate start: " + migratingKey  + " frontier: " + frontier.get(migratingKey));
    }
    if (affectedKeys.containsKey(event.f0) && affectedKeys.get(event.f0) && migratingKey.equals(event.f0)) {
      // try to delete buffer
      bufferOrRelease(rulesState, out, event);
    } else if (affectedKeys.containsKey(event.f0) && affectedKeys.get(event.f0) && !migratingKey.equals(event.f0)) {
...
//        return;
  ...
  private void bufferOrRelease(Map<String, Integer> rulesState,
...
//        }
        ...
        System.out.println("++++++ clear buffer start: " + key);
        ...
        System.out.println("++++++ clear buffer complete: " + key);
...
//        }
        ...
        System.out.println("++++++buffered: " + event
                ...
                + " cur state: " + stateTs
                + " offset: " + (frontier.get(event.f0) - stateTs) / 1000_000_000);
...
//            ? rulesState.getOrDefault(event.f0, -1)
//            : oldBroadcastState.getOrDefault(event.f0, -1);
//
//    // assign a key that exactly mapped to the assigned keygroup.
//    String assignedKey = assignedKeyGroup == -1 ? event.f0 :
////            !keyGroupToKeyMap.containsKey(assignedKeyGroup) ? event.f0 : keyGroupToKeyMap.get(assignedKeyGroup);
//              keyGroupToKeyMap.getOrDefault(assignedKeyGroup, event.f0);
...
//    System.out.println("key: " + event.f0 + " normal: " + (rulesState != null) + " frontier: " + curTs);
    ...
    if (affectedKeys.containsKey(event.f0) && affectedKeys.get(event.f0) && migratingKey.equals(event.f0)) {
      ...
      // assume that the state is locally shared among upstream and downstream
...
//              globalState.get(event.f0).split("\\|")[0]));
      out.collect(new Keyed<>(eventWithTs, KeysExtractor.getKey(assignedKey), payload));
